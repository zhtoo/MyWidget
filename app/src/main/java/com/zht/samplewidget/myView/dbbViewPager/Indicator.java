package com.zht.samplewidget.myView.dbbViewPager;
...
public class Indicator extends View {
    ...
    private int bule = Color.argb(255, 23, 140, 219);
    private int orage = Color.argb(255, 239, 128, 0);
    private int gray = Color.argb(255, 204, 204, 204);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        centerWidth = width / 2;
        dynamicLengthFirat = 0;
        dynamicLengthSecond = 3 * width / 4;
    }
    ...
    private void initPaint() {
        ...
        firatPaint.setColor(bule);//设置画笔的颜色
        firatPaint.setStrokeWidth(2);//设置画笔的宽度
        //创建画笔的对象,用于画空心圆
        secondPaint = new Paint();
        secondPaint.setAntiAlias(true);
        secondPaint.setStyle(Paint.Style.FILL);
        secondPaint.setColor(gray);
    ...
    public void setoffset(int position, float positionOffset) {
        ...
        if (positionOffset != 0) {
            if (positionOffset - offset > 0) {//向左
               ...
               /* int color = firatPaint.getColor();
                int red = Color.red(color);
                int green = Color.green(color);
                int blue = Color.blue(color);
                Log.d(TAG,"向左  red:"+red+"  green:"+green+"   blue:" +blue);*/

                firatPaint.setColor(Color.argb(255,
                        (int) (23 + 181 * positionOffset),
                        (int) (140 + 64 * positionOffset),
                        (int) (219 - 15 * positionOffset)
                ));
                secondPaint.setColor(Color.argb(255,
